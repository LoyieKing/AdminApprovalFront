...
export default class App extends React.Component {
    ...
    componentDidMount() {
        ...
        getMenus(userId)
            .then(res => {
                const plainMenus = (res ?? []).filter(({ key }) => loginUser.username == "admin" || (loginUser.permissions?.indexOf(key) ?? -1) != -1);
                const permissions = loginUser.permissions ?? []
                const userPaths = [];

                const { menuTreeData } = getMenuTreeDataAndPermissions(plainMenus);

                plainMenus.forEach(({ type, path, code, key }) => {
                    if (loginUser.username == "admin" || (loginUser.permissions?.indexOf(key) ?? -1) != -1) {
                        if (path) {
                            userPaths.push(path)
                        }
                    }
